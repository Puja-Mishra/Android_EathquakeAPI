...
public final class QueryUtils {
    ...
    /**
     * Log Tag string for logging error in the Monitor
     */
    ...
    public static ArrayList<EarthQuake> extractEarthquakes(String jsonResponse) {
        ...
        try {
            ...
            //rootObject=rootObject.getJSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray newArray = rootObject.getJSONArray("features");


            for (int i = 0; i < newArray.length(); i++) {
                JSONObject arrayObjects = newArray.getJSONObject(i);
                JSONObject properties = arrayObjects.getJSONObject("properties");
                String mag = properties.getString("mag");
                String place = properties.getString("place");
                String time = properties.getString("time");
                String url = properties.getString("url");
                //Log.v("inside Query Utils",mag);
                //Log.v("inside Query Utils",place);
                //Log.v("inside Query Utils",time);
                Long eTime = Long.parseLong(time);
                Date dateObject = new Date(eTime);
                SimpleDateFormat dateFormat = new SimpleDateFormat("MMM dd, yyyy:HH mm a");
                String NewDate = dateFormat.format(dateObject);
                earthquakes.add(new EarthQuake(mag, place, NewDate, url));
            }
